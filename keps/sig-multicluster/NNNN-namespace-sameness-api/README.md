...
clusters. Consumers do not read parent requests or status; their inputs are
published slices and the referenced inventory.
...
Cover every row of the acceptance table, including conflicting unpublished
requests, an existing member with a competing request, and partially
conflicting candidate sets. Permute parent evaluation order and use both
selector and external candidates; unpublished competitors must not gain a
winner from iteration order. Test singleton published groups in conflict
checks and membership in different namespace names. Verify idempotent,
order-independent reference union and the following identity cases:
...
- Run one producer managing multiple inventories, then separate producers
  managing those inventories. Verify equivalent membership within each
  inventory and no acceptance checks or writes across their assigned scopes.
  Referenced profiles in another namespace remain readable where authorized.
- Run multiple replicas and concurrent parent reconciliations of one producer.
  Verify coordinated inventory-wide acceptance and publication. Replace the
  producer after ending the old producer's writes; verify that the replacement
  synchronizes all requests and sources before accepting additions.
- Combine selector and external candidate sources in one inventory, then
  change a parent's source. Verify the same acceptance rules and reconciliation
  of existing slices, with no independent external slice writer.
- Before an acceptance evaluation, create two parents requesting the same
  previously unassigned `cluster-c/billing`. Verify that neither publishes
  that member, both report `MembershipConflict`, and other accepted or
  non-conflicting members remain usable. Repeat with different parent orders,
  producer restarts, and conflicting candidates from external sources.
  An administrator removes the request from one parent; verify that the sole
  remaining requester is accepted without resubmission. Repeat resolution by
  deleting a parent and by updating external desired membership.
- Make one parent's candidate source unavailable while another requests a
  new member. Verify that new additions in that inventory are withheld until
  the producer can compare all candidates. Preserve established membership
  subject to known invalidations, process known removals, and allow an
  independently synchronized inventory to continue accepting additions.
- Start with A/B; request C/D where only C conflicts. Verify that A/B remain,
  ...
  An administrator changes the existing owner's request to remove C; verify
  withdrawal of its references before C is accepted for the waiting parent.
- Publish `foo-app` on A/B and `bar-app` on B/C. Verify that both groups are
  accepted, including B's participation in both namespace names, without
  asserting `foo-app` sameness between A and C or `bar-app` sameness with A.
...
- Inject invalid slices publishing an overlapping cluster/name pair.
...
- Change a referenced profile's ID without changing selectors or slices.
  ...
  of a published conflict. Verify that the producer reports `MembershipConflict`
  and does not choose a winner or cycle the valid, still-requested references,
  while consumers exclude the affected groups. Have an administrator correct
  the inventory data and verify recovery. Repeat with label addition, removal,
  and an empty value, through watch updates and recovery from a relist.
...
- Reject unauthorized parent spec, status, slice, and inventory writes by
  member or consumer identities. Reject producer writes and deletions in
  inventories outside its assignment and writes using the old producer's
  credentials after a handoff.
...
enabling it. Integration configuration is described in
[Notes/Constraints/Caveats](#notesconstraintscaveats).
