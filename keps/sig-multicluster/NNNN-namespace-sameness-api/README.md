...
- [Proposal](#proposal)
  ...
  - [Deployment Models](#deployment-models)
    - [Resources on a Hub Cluster](#resources-on-a-hub-cluster)
      - [Global Hub](#global-hub)
      - [Regional Hubs](#regional-hubs)
    - [Resources on Member Clusters](#resources-on-member-clusters)
...
published membership without evaluating selection policy. Slices bound the
size of individual objects and allow membership changes without rewriting an
entire large group.
...
integrate this API as described in
[Notes/Constraints/Caveats](#notesconstraintscaveats).
...
participating clusters separately for each namespace name.

An organization may need different sets of clusters for different namespaces.
For example, application teams may use `foo-app` in `cluster-a` and
`cluster-b`, and `bar-app` in `cluster-b` and `cluster-c`. The finance team
uses `billing` in `cluster-c` and `cluster-d`. Creating a namespace named
`billing` in `cluster-a` should not make it part of the finance team's namespace.

![The foo-app band spans cluster-a and cluster-b; the bar-app band spans cluster-b and cluster-c. The finance band encloses billing in cluster-c and cluster-d; the same-named billing Namespace in cluster-a sits outside it.](./namespace-boundaries.svg)
...
optionally, selectors over ClusterProfiles. The inventory's producer evaluates
its requests together, checks for conflicts, and publishes accepted membership.
...
Consumers derive relationships from the published slices and referenced
inventory as specified in [Consumer Processing](#consumer-processing).
...
| `bar-app` | `bar-app` | `cluster-b`, `cluster-c` |
| `finance` | `billing` | `cluster-c`, `cluster-d` |

`cluster-b` participates in both application groups, with a different peer
for each namespace. This does not relate `foo-app` to `bar-app` or give
`foo-app` membership in `cluster-c`.
...
An administrator removes `cluster-c` from `team-b`'s request if it
should move to `team-a`. Once the producer withdraws that membership, it can
add `cluster-c` to `team-a` without resubmitting the latter's request.
...
![An administrator assigns foo-app imports in cluster-a and cluster-b to MCS controller A, and bar-app imports in cluster-b and cluster-c to MCS controller B. Each controller uses the membership for its assigned namespace.](./consumer-selection.svg)

The read scope for each controller is defined in
[Consumer Processing](#consumer-processing).
...
inventory's producer collects those candidates together with selector results
for other parents. External sources do not independently publish slices.
Their schemas and access protocols are internal to the producer.

### Deployment Models

Administrators choose where to host NamespaceSameness, NamespaceSamenessSlice,
and ClusterProfile resources and configure each consumer's trusted API
endpoint and resource namespace.

We recommend hosting these resources on a hub cluster to give consumers a
shared API endpoint.

The examples store these API resources in a namespace named `inventory`.
Administrators can choose another name. The published groups describe
membership for the `foo-app` namespace, with an MCS controller running in
each member cluster.

The placements below use the producer assignments in
[Ownership and Security](#ownership-and-security) and the read behavior in
[Consumer Processing](#consumer-processing).

#### Resources on a Hub Cluster

![A hub's inventory namespace contains NamespaceSameness foo-app, its Slice, and ClusterProfiles for cluster-a and cluster-b. The MCS controller in each member cluster reads the hub's Slices and ClusterProfiles. The blue band encloses foo-app in both member clusters.](./deployment-hub.svg)

An administrator places the NamespaceSameness request and ClusterProfiles in
the hub's `inventory` namespace. The assigned producer publishes slices there.
The MCS controllers in `cluster-a` and `cluster-b` read the hub's slices and
inventory while managing resources in their own clusters.

Deployments can use an existing management cluster or dedicate a cluster to
the API. Separate hubs can serve different administrative domains or regions.

The global and regional hub examples use the same two separate `foo-app`
groups: `cluster-a` and `cluster-b` in Region A, and `cluster-c` and `cluster-d`
in Region B. The hub boxes show the slices and ClusterProfiles that consumers
read; NamespaceSameness parents and producers are omitted.

The placement of hubs does not determine membership. The two groups remain
separate in both examples; placing them on one hub does not join them. A group
spanning regions can also be published on one hub.

##### Global Hub

![One global hub stores two separate foo-app groups and all four ClusterProfiles in its inventory namespace. The MCS controllers in cluster-a and cluster-b in Region A and cluster-c and cluster-d in Region B all read that namespace. Blue and purple bands mark the two separate groups.](./deployment-global-hub.svg)

With a global hub, one `inventory` namespace holds both groups and all four
ClusterProfiles. All four controllers read that inventory.

##### Regional Hubs

![Region A contains hub-a and the MCS controllers in cluster-a and cluster-b that read it. Region B contains hub-b and the controllers in cluster-c and cluster-d that read it. Each hub's inventory namespace holds its regional foo-app group's Slice and both referenced ClusterProfiles. Blue and purple bands mark the two separate groups.](./deployment-regional-hubs.svg)

With regional hubs, `hub-a` stores the Region A group and its profiles, and
`hub-b` stores the Region B group and its profiles. Each controller reads its
regional hub.

#### Resources on Member Clusters

![Each member cluster has an inventory namespace containing its own NamespaceSameness foo-app, Slice, and ClusterProfiles for both cluster-a and cluster-b. Each MCS controller reads its local API. The independently published groups describe the same foo-app relationship across the two clusters.](./deployment-member-clusters.svg)

Each member cluster hosts the APIs and an `inventory` namespace. In this
example, an administrator distributes a common membership configuration, and
a producer in each cluster publishes the local slices. Both local inventories
contain profiles for `cluster-a` and `cluster-b`, so each consumer can resolve
the relationship using only its local API.

Local parents and ClusterProfiles have their own UIDs. When projecting groups
to another API server, the producer must resolve ClusterProfile references and
parent ownership on that server.

The deployment's producers or distribution mechanism coordinate membership
updates and deletions across clusters. This API defines neither that
synchronization protocol nor simultaneous application by consumers.

### Risks and Mitigations

An incorrect membership decision can combine resources belonging to different
teams. The inventory's producer withholds conflicting requests before
publication, as specified in [Accepting Clusters](#accepting-clusters).
Consumers also check for invalid published membership, including conflicts
caused by inventory identity changes or writes outside the assigned producer.
...
but delays revocation, as described in
[Read Failures and Recovery](#read-failures-and-recovery).
...
- **Inventory**: ClusterProfile objects in one namespace on one Kubernetes
  API server, following [KEP-4322]. NamespaceSameness requests and their slices
  are stored in that namespace.
- **Authority scope**: The configured API endpoint and inventory namespace
  whose groups a consumer trusts.
...
- **Producer**: The controller assigned to evaluate an inventory's requests
  and publish their status and slices, including candidates from external
  policy sources.
...
The Go definitions below include selected validation markers. Implementations
must enforce all requirements in [Validation](#validation), including those
not expressed by these markers.
...
type NamespaceSamenessSpec struct {
    ...
    // Omitted or empty lists use the inventory producer's external source.
    // +optional
    // +listType=atomic
...
| --- | --- |
| Omitted or `[]` | The inventory's producer obtains candidates from its configured external source |
...
Consumers read this field directly as specified in
[Consumer Processing](#consumer-processing).
...
For selector-based selection, the producer publishes references in the parent's
namespace. External candidates may reference profiles in other namespaces on
that server if both producer and consumer are authorized to read them.
Reference validation and the identity of the represented member follow
[Cluster Identity](#cluster-identity).
...
To use an external policy source configured for the inventory's producer, the
...
This last manifest needs a configured external source to supply candidates.
Until that source is available, the producer reports `SourceUnavailable`
as described in [Status Conditions](#status-conditions).
Installing another controller must not override the inventory's producer
assignment.
...
a failed inventory read as a missing label. Producers follow
[Accepting Clusters](#accepting-clusters) when inputs are unavailable;
consumers follow [Read Failures and Recovery](#read-failures-and-recovery).
...
A group with zero or one participating cluster establishes no cross-cluster
relationship. [Consumer Processing](#consumer-processing) defines how a
consumer determines which groups it can use.
...
The producer withholds conflicting requests as specified in
[Accepting Clusters](#accepting-clusters). The checks here protect consumers
when published data is already inconsistent, for example after a member ID
change or an unauthorized write. If `team-a` contains `billing` in A/B and
`team-b` contains `billing` in B/C, both publish
`cluster-b/billing`.

The consumer excludes both groups, not just the duplicated cluster. It does
not derive A-to-C sameness by combining the groups.
...
The consumer must not choose a winner based on watch arrival order, parent or
profile creation timestamps, a condition timestamp, or its cached previous
membership.

The producer reports the published conflict and requires an administrator to
correct the requests or inventory data. It must not choose a winner or
repeatedly withdraw and re-add still-requested, valid references solely to
resolve that conflict. Consumers resume using the groups once they observe
that the published overlap has been removed; status does not override their
checks.
...
property values use the property value limits. [Cluster Selection](#cluster-selection)
defines these limits and each operator's value cardinality. Invalid requirements
are rejected, not treated as empty selectors.
...
its parent. Schema validation cannot compare these separate objects.
...
Before accepting additions, the inventory's producer synchronizes its views
of all parents, slices, and referenced inventory. It evaluates candidates for
every non-deleting parent, including candidates supplied by external sources.
It MUST compare these requests together before publishing any new acceptance;
processing one parent first must not give it priority over competing requests
in the same evaluation.

Checks use the pair of resolved member identity and target namespace name.
Identity resolution follows [Cluster Identity](#cluster-identity), so different
profiles representing the same member can conflict. Existing membership is
determined from valid references in non-deleting slices. These published claims
remain part of the check even when their parent is deleting or absent.

| Published membership for a member/namespace pair | Current requests | Result |
| --- | --- | --- |
| None | One parent requests the pair | Accept it for that parent |
| None | Multiple parents request the pair | Withhold it from every requesting parent |
| One group | Its parent still requests the pair | Preserve the existing membership and withhold any requests from other parents |
| One group | Its parent no longer requests the pair or is deleting or absent | Withdraw or await cleanup of the old references before considering new requests |
| Multiple groups | Any | Apply [Overlapping Groups](#overlapping-groups); do not choose a winner |

Concurrent requests in this table are requests observed together before the
pair has been accepted into a slice. A request first observed after an
acceptance is an addition competing with existing membership. Reordering
parents, restarting the producer, or retrying unchanged requests MUST NOT
select a winner among requests that remain unpublished and competing.

For example, if `team-a` and `team-b` both request the previously unassigned
`cluster-c/billing`, the producer adds it to neither group's slices. Both
parents report `ClustersResolved=False` with reason `MembershipConflict`.
Non-conflicting candidates can still be accepted, and existing membership
continues to be published. Consumers have no new membership to process for
the withheld pair.

An administrator resolves the conflict by changing the parents' selectors or
external desired membership, or by deleting an unwanted parent. For example,
removing `cluster-c` from `team-b`'s request leaves `team-a` as the sole
requester, allowing the producer to add it on reevaluation. Deleting a slice
alone does not change either request and is not a durable resolution.

The producer retries pending work when requests, inventory, or published
membership change. If it cannot evaluate any parent's candidates because an
inventory or external-source read fails, it MUST withhold new additions across
that inventory until it can perform the complete comparison. It must not
interpret the unavailable source as an empty candidate set. Existing membership
is retained subject to known invalidations, and known removals proceed
independently of pending additions.
...
to every profile representing it. The producer does not reassign the member
while those blocking published references remain. A conflict elsewhere must
not block removal. These updates are observed asynchronously by consumers.

Changing between selector-based and external selection changes the candidate
source used by the inventory's producer. The administrator configures the new
source, and the producer reconciles existing slices against it. Omitting
selectors does not, by itself, clear previously published membership.
Replacing the producer follows [Ownership and Security](#ownership-and-security).
...
| --- | --- | --- |
| `True` | `Resolved` | Published members match the evaluated candidate member set, with no pending candidates, unselected published references, or published overlaps |
| `False` | `MembershipConflict` | Candidates are withheld because another parent requests or publishes the same member/namespace pair, or published membership already overlaps another group |
| `False` | `Reconciling` | Membership changes are being published |
| `Unknown` | `SourceUnavailable` | A source needed to evaluate this parent or check its additions against other requests in the inventory is unavailable |
...
Conditions report producer progress to operators. Consumers determine usable
membership as specified in [Consumer Processing](#consumer-processing).
...
allow Kubernetes garbage collection to remove its slices asynchronously.
Consumers process the remaining slices as specified in
[Consumer Processing](#consumer-processing).
...
member identities occur in one eligible group. It must not infer membership
from matching names or the presence of Namespaces or Services in member
clusters. Consumers read namespace names directly from published slices and
resolve members from the referenced inventory. They do not fetch parents or
check their existence, spec, or status. A false, unknown, or absent parent
condition does not disable published membership, and generation equality is
not a publication barrier. Eligible slices remain usable while additions are
pending or the parent is deleting or absent.
...
writers. Each inventory MUST have at most one assigned logical producer for
all NamespaceSameness status and slice writes. One producer may manage multiple
inventories, or administrators may assign a different producer to each
inventory. These assignments do not combine inventories or their groups.

Within an inventory, the producer MUST coordinate its replicas and processing
of different parents so that acceptance and publication cannot proceed
independently. External selection sources feed candidates into this producer;
they do not gain separate slice-writing authority. The deployment configures
the assignment. `producerName` is diagnostic and does not select a controller.

To replace a producer, the administrator stops its reconciliation and ends
its write access before enabling the replacement. The replacement synchronizes
all requests, published slices, and candidate sources before accepting additions.
...
| Producer | Read all parents and slices in its assigned inventories and all referenced inventory; update status and create/update/delete slices there | Write membership in an inventory it is not assigned to manage |
...
`multicluster.x-k8s.io` API group for parents in its assigned inventories. The
...
Namespace-scoped grants restrict producer writes to assigned inventories.
Policy writers may share an inventory, but all their requests pass through
its producer; parent-name labels do not grant independent slice ownership.
The assignment does not require a webhook solely to enforce the
parent-association recommendation.
...
[Services without selectors] provide a precedent for populating resolved
membership from an external source. [EndpointSlice] provides the
...
The following cases define coverage shared by the unit and integration suites.
Unit tests check validation and membership calculations; integration tests check
API enforcement, observation, and reconciliation of the same cases.

| Area | Cases | Specification |
| --- | --- | --- |
| Validation | Required fields, namespace names and immutability on both kinds, parent name length, complete references, and 0/100/101 entries per slice before member deduplication | [Validation](#validation) |
| Selection | Label and property maps and expressions, operator and value constraints, empty inputs, and inventory scope | [Cluster Selection](#cluster-selection) |
| Identity | Duplicate member IDs, missing/empty IDs, ID changes, missing profiles, stale UIDs, and separate API servers and authority scopes | [Cluster Identity](#cluster-identity) |
| Association | Missing or invalid metadata, changed owner association, and slices disagreeing on the namespace name | [NamespaceSamenessSlice](#namespacesamenessslice) |
| Acceptance and conflicts | Every acceptance-table row, partial acceptance, singleton groups, independent namespace names, and published overlaps | [Accepting Clusters](#accepting-clusters), [Overlapping Groups](#overlapping-groups) |
| Lifecycle | Empty groups, incremental updates, duplicate references, parent and profile deletion/recreation, and downstream convergence | [Incremental Updates](#incremental-updates), [Deletion and Cleanup](#deletion-and-cleanup), [Consumer Processing](#consumer-processing) |
| Read recovery | Partial lists, expired watches, outages, initial synchronization, and recovery by relisting | [Read Failures and Recovery](#read-failures-and-recovery) |
| Authorization | Parent, status, slice, and inventory access; producer assignments and handoff; consumers without parent access | [Ownership and Security](#ownership-and-security) |
...
order. Producer tests also need multiple inventories, coordinated replicas,
and controllable external candidate sources. MCS tests need an implementation
that explicitly integrates NamespaceSameness.

##### Unit tests

Test validation, selector evaluation, identity resolution, slice grouping,
cluster acceptance, and reference union using the shared cases above. Exercise
acceptance independently from consumer conflict detection.

For both selector kinds, cover map-only, expression-only, and combined
requirements, including:

- AND within an entry and OR across entries; reordered and duplicate entries.
- Every operator with present and absent keys; invalid operators, empty
  `In`/`NotIn` values, and non-empty `Exists`/`DoesNotExist` values.
- Omitted and empty lists, empty entries, omitted/null/empty nested selectors,
  and non-empty selectors matching no profiles.
- Property strings invalid as label values, including lengths above 63
  characters, without interpreting them as numbers or templates.
- Failed inventory reads without treating them as absent keys or empty sets.

Verify idempotent, order-independent membership calculations for empty,
singleton, and larger groups. Different profiles for one member must not
establish a cross-cluster relationship by themselves. Keep claims from groups
with inconsistent namespace names in conflict checks before excluding them.

Cover equal member IDs across profile names, UIDs, and inventory namespaces,
and isolation across API servers and authority scopes. Missing or empty IDs
use object identity without comparing an ID value to an object UID. An
unreferenced profile must not join a group, and an older unselected profile
must not replace a selected candidate. A missing profile or UID mismatch
invalidates only that reference, even if another profile has the same member
ID. A failed read is not evidence of a missing profile or label.

Cover every row of the acceptance table with selector and external candidates.
Permute parent evaluation order for competing unpublished requests. Verify
preservation of existing membership and acceptance of non-conflicting additions.
For published overlaps, check claims before representative-profile selection,
including different and equal profile creation timestamps, singleton groups,
and membership under different namespace names.

##### Integration tests

Use a real API server to verify schema enforcement and reconciliation for the
shared cases, with the following setups and transitions:

- Exercise the validation boundaries on both kinds, including missing, empty,
  invalid, replaced, and removed namespace names. Verify that producers copy
  `spec.namespace` into slices, rejected writes preserve membership, and parent
  selector and slice reference changes remain allowed.
- Run consumers without get/list/watch permission on parents. Start with a
  parent without slices, then publish membership with absent, false, and unknown
  conditions. Verify startup, reconciliation, and recovery using only slices
  and inventory, with eligible membership usable regardless of conditions.
- Run two consumers on the same membership with separate downstream resources.
  Verify equal relationships and writes limited to their assigned resources.
  A consumer managing only one of two overlapping groups must still detect both.
- Compare one producer managing multiple inventories with separate producers.
  Verify equivalent membership and acceptance checks and writes confined to
  assigned inventories. Allow authorized external references across namespaces;
  selector evaluation must stay within the parent's namespace despite broader
  read permissions. Observe profile registration, deletion, label, and property
  changes.
- Run coordinated replicas and concurrent parent reconciliations. End the old
  producer's writes before a handoff and verify that the replacement synchronizes
  all requests and sources before accepting additions.
- Combine selector and external sources, then change a parent's source. Verify
  reconciliation of existing slices by the assigned producer. Create competing
  requests for unassigned `cluster-c/billing` before evaluation; neither publishes
  the pair and both report `MembershipConflict`. Repeat with different parent
  orders and producer restarts. Resolve through selector changes, external desired
  membership, or parent deletion; the sole requester proceeds without resubmission.
- Start with A/B and request C/D where C already belongs to another group.
  Verify that A/B remain, D is accepted, C is pending with `MembershipConflict`,
  and C's old references are withdrawn before reassignment. Exercise independent
  namespace names using `foo-app` on A/B and `bar-app` on B/C. Also select A/B/C
  for `catalog` and `billing` while another group owns `billing` on C; only the
  latter addition must remain pending.
- Make one parent's source unavailable while another requests an addition.
  Withhold additions across that inventory, retain membership subject to known
  invalidations, process known removals, and allow independent inventories to
  continue. Resume comparison when the source recovers.
- Replay the identity and association cases through watch updates and relists.
  Include missing, inconsistent, and changed parent associations; deleting,
  recreating, or invalidating profiles; and member ID addition, replacement,
  removal, or an empty value without selector or slice changes. Inject published
  overlaps and namespace disagreement, including overlap with another group.
  Verify exclusion of invalid references, withdrawal from old associations, and
  exclusion of whole groups for namespace disagreement or overlap. Verify
  diagnostics, usable non-conflicting membership, and recovery after correction.
  Producers report `MembershipConflict` without choosing a winner or cycling
  valid, still-requested references.
- Exercise A/B-to-B/C slice updates with different event orders, including A/B/C
  and B-only intermediate views without a status barrier. Move references across
  slices and publish multiple profiles for one member; withdraw membership only
  after its last valid reference disappears.
- Delete a parent while retaining slices, both with a visible deletion timestamp
  and with immediate deletion. Verify continued use by existing and restarted
  consumers. Recreate the name with a new UID; old and new groups must participate
  in conflict checks, and producers must withhold additions blocked by old slices.
- Mark the final slice for deletion while a test finalizer keeps it visible,
  then remove it. Verify removal of applied relationships in both states, and
  repeat by clearing references and by relisting an empty slice set.
- Repeat reconciliation and restart consumers with existing downstream resources.
  Verify convergence, repair of downstream drift, and removal of withdrawn
  relationships without duplicate effects.
- Reject unauthorized parent spec, status, slice, and inventory writes by member
  or consumer identities. Reject producer writes and deletions outside assigned
  inventories and writes using the old producer's credentials after a handoff.
- Interrupt paginated lists, expire watches, and disconnect established consumers
  while starting new ones. Verify that partial lists neither activate membership
  nor imply deletion, established consumers retain state subject to known
  invalidations, and fresh consumers assert no relationship. A complete relist
  replaces the affected cache and removes absent slices and profiles.
- Exercise enablement, disablement, reenablement, and every served API version.
  Verify preservation of selectors, namespace names, parent ownership, profile
  references, and resolved member identities through conversion.
...
Verify that a consumer does not aggregate its exports twice. Inject invalid
slices publishing those references in separate groups for the same namespace
and verify that the consumer excludes both groups from aggregation.
...
Include competing unpublished requests, invalid overlapping published data,
many pending candidates, underfilled slices, abandoned empty or orphaned slices,
and sustained inventory churn. Vary the number of profiles per member, including
across inventory namespaces, and change member ID labels under load. Include
...
- Provide a producer supporting selector and external candidate sources, and
  a reference consumer.
- Pass selection, partial acceptance, administrator conflict resolution,
  producer assignment, lifecycle, authorization, and MCS boundary tests.
- Complete the proposed 100,000-cluster metadata-scale test and publish
  initial-publication and incremental-update results.
- Document inventory assignments, intermediate membership, diagnostics,
...
- Validate compatibility across supported API versions, upgrade, downgrade,
  reenablement, and producer handoff.
- Establish measured capacity limits, operational indicators, and latency
  objectives for the reference implementation, and meet those objectives.
...
- Complete all functional, security, monitoring, and testing requirements,
  and resolve known issues and gaps before beta.

#### GA

- Demonstrate sustained production use by independent implementations,
  including a record of meeting the documented scale and operational objectives.
- Resolve issues and gaps identified through beta usage and feedback.
- Document the stable API's deprecation policy.
...
Consumers apply the identity rules in
[NamespaceSamenessSlice](#namespacesamenessslice) and
[Cluster Identity](#cluster-identity) across served versions.
...
- [x] Other
  ...
  - Configure producer permissions for its assigned inventories and replica
    coordination as described in [Ownership and Security](#ownership-and-security).
...
Default behavior changes only for consumers with the integration enabled.
Operators must reconcile and inspect the intended groups before enabling it,
following [Notes/Constraints/Caveats](#notesconstraintscaveats).
...
reenabling consumers. Consumers then follow the startup procedure in
[Consumer Processing](#consumer-processing).
...
The assigned producer and its candidate sources are needed for new acceptance.
A source failure can withhold additions throughout its inventory, as specified
in [Accepting Clusters](#accepting-clusters), while published membership remains
available to consumers. Separate inventories can continue independently.

Admission services are dependencies only where a deployment uses additional
checks. Credential, network, and MCS dependencies belong to the integrating
implementation.
...
inventory manager or producer using external sources may use a cloud provider
as part of its own data collection or policy evaluation.
...
Reconciliation stops and revocations may be delayed. Consumers follow
[Read Failures and Recovery](#read-failures-and-recovery) for retained state,
startup, and recovery after the outage.

###### What are other known failure modes?

For a pending addition, inspect `ClustersResolved` and the reported conflicting
member and parents, including requests that have not published any slices.
Follow the administrator resolution procedure in
[Accepting Clusters](#accepting-clusters).
...
Check that a parent without selectors has a configured external source and
that sources for other parents are available for the inventory-wide comparison.
...
across all groups in an authority scope. Each inventory depends on its producer
to compare all requests before accepting additions. An unavailable candidate
source can delay other parents' additions, and conflicting requests require
administrator intervention.

Applications requiring atomic boundary changes or bounded revocation need
additional coordination because of the behavior described in
[Incremental Updates](#incremental-updates) and
[Read Failures and Recovery](#read-failures-and-recovery).
...
and consumer, and CI for integration, e2e, and scale tests. Deployment guidance
must cover inventory assignments and producer handoff.
